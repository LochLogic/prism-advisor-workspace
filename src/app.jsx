...
/* ─── Topbar ──────────────────────────────────────────────────────── */
/* ─── Help drawer (round 23) - searchable advisor guides ──────────────
   Content = window.__pxGuides, generated by build.mjs from docs/guides/*.md.
   ONE markdown source renders BOTH this drawer and the printable
   /guides/<slug>/ page (the "save as searchable PDF" training artifact the
   industry advisor asked for). Advisor bundle only - build.mjs never embeds
   guides into the portal bundle. */
// Guide HTML comes from build.mjs rendering repo markdown (trusted source),
// but defense-in-depth still applies: escape everything, then re-allow ONLY
// the bare attribute-less structural tags the generator emits. (The shared
// sanitizeHtml whitelist is narrower - b/i/em/strong/br - by design; widen
// THIS list, not that one, if the guide markdown subset ever grows.)
const _guideHtml = (s) => escapeHtml(s)
  .replace(/&lt;(\/?)(b|i|em|strong|br|p|ul|ol|li|h1|h2|h3|code|table|thead|tbody|tr|th|td)&gt;/gi, '<$1$2>');

const HelpDrawer = ({ onClose }) => {
  const guides = window.__pxGuides || [];
  const [q, setQ] = React.useState('');
  const [slug, setSlug] = React.useState(guides[0]?.slug || null);
  const needle = q.trim().toLowerCase();
  const matches = needle
    ? guides.filter(g => g.title.toLowerCase().includes(needle) || (g.md || '').toLowerCase().includes(needle))
    : guides;
  const active = matches.find(g => g.slug === slug) || matches[0] || null;
  const hits = (g) => needle ? Math.max(0, (g.md || '').toLowerCase().split(needle).length - 1) : 0;
  return (
    <>
      <div className="px-drawer-backdrop" onClick={onClose} />
      <aside className="px-drawer" role="dialog" aria-modal="true" aria-label="Help and guides">
        <div className="px-drawer-head">
          <div>
            <div className="px-eyebrow">Help &amp; training</div>
            <div style={{ fontFamily: 'var(--serif)', fontSize: 18, color: 'var(--ink)' }}>Advisor guides</div>
          </div>
          <button className="px-modal-close" onClick={onClose} aria-label="Close">×</button>
        </div>
        <div className="px-drawer-body">
          <input type="search" className="px-input" placeholder="Search the guides…" autoFocus
            value={q} onChange={(e) => setQ(e.target.value)} style={{ marginBottom: 12 }} />
          {guides.length === 0 && (
            <div style={{ color: 'var(--ink-faint)', fontStyle: 'italic', fontSize: 12 }}>
              No guides bundled - check docs/guides/ and the build.
            </div>
          )}
          {(needle || guides.length > 1) && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
              {matches.map(g => (
                <button key={g.slug} className={`px-btn px-btn-sm ${active?.slug === g.slug ? 'px-btn-primary' : 'px-btn-ghost'}`}
                  style={{ padding: '3px 10px' }} onClick={() => setSlug(g.slug)}>
                  {g.title}{needle ? ` · ${hits(g)} match${hits(g) === 1 ? '' : 'es'}` : ''}
                </button>
              ))}
              {needle && matches.length === 0 && (
                <span style={{ color: 'var(--ink-faint)', fontSize: 12 }}>Nothing matches "{q}" - try a shorter term.</span>
              )}
            </div>
          )}
          {active && (
            <>
              <a className="px-btn px-btn-sm px-btn-ghost" href={`/guides/${active.slug}/`} target="_blank" rel="noopener"
                style={{ marginBottom: 12, display: 'inline-flex' }}>
                <Icons.FileText size={11} /> Printable version (save as PDF)
              </a>
              <div className="px-help-md" dangerouslySetInnerHTML={{ __html: _guideHtml(active.html) }} />
            </>
          )}
        </div>
      </aside>
  ...
  return (
    <header className="px-topbar">
      ...
      <div className="px-topright">
        ...
        {(role === 'advisor' || role === 'admin') && (
          <button className="px-icon-btn" onClick={() => setHelpOpen(true)}
            aria-label="Help and guides" title="Help & guides">
