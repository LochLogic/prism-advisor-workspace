...
const guides = readdirSync('docs/guides').filter(f => f.endsWith('.md')).sort().map(f => {
  const raw = readFileSync(`docs/guides/${f}`, 'utf8');
  // Optional `<!-- audience: client -->` marker (default advisor) sets the
  // printable page's bar label so a client-facing handout doesn't read
  // "Advisor guide". Stripped before mdToHtml so it never renders as text.
  const audience = (raw.match(/^<!--\s*audience:\s*(\w+)\s*-->\s*$/im) || [])[1] || 'advisor';
  const md = raw.replace(/^<!--\s*audience:\s*\w+\s*-->\r?\n?/im, '');
  const title = ((md.match(/^# (.+)$/m) || [])[1] || f.replace(/\.md$/, '')).trim();
  return { slug: f.replace(/\.md$/, ''), title, audience, md, html: mdToHtml(md) };
});
...
<body>
<div class="pxg-bar"><span>Prism | ${g.audience === 'client' ? 'Client' : 'Advisor'} guide</span><span class="pxg-hint">Use your browser's Print to save this as a searchable PDF</span></div>
${g.html}
</body></html>`;
