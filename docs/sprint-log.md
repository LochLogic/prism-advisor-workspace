...
## 2026-06-11 - Round 22: industry-advisor meeting feedback (five UI fixes + seven design tracks)

PR #73. Build · smoke · calc · lint green. **No migration, no secrets, no
money** - frontend + docs only.

Source: the user's working session with an industry advisor (notes relayed
2026-06-11). Straightforward items shipped; think-items roadmapped.

**What shipped**
- **Leading-zero input fix, app-wide.** Root cause: 43 raw controlled
  `<input type="number">` in the phase tools - clearing one commits 0, React
  renders "0", the next keystroke makes "05", and React's loose number-input
  comparison keeps it on screen. `NumInput` (the draft-holding input the
  Numbers drawer already used) moved to `components.jsx`, hardened to collapse
  a digit typed after a lone "0", published on `window`, and every calculator
  input converted to it. Verified in-browser: clear → empty field (not "0"),
  "0" then "5" → "5".
- **Numbers-panel section headers promoted.** The 15 ledger section headers
  (Household, Income, Housing, … Goals) get `px-eyebrow is-section`: 11.5px,
  full ink, accent tick before the text - clear breaks while scrolling. The
  drawer title and the W-2/1040 import labels stay plain eyebrows.
- **P03 avalanche milestone → tool link.** "Apply avalanche payoff above 6%
  APR" now carries `tool: 'advanced'` ("See tool below" jump), like its peers.
- **P06 milestone 3 + tool order.** "Enable systematic tax-loss harvesting"
  keeps its Discuss button and gains "See tool below" via a new additive
  `seeTool: true` task flag (client-portal renders the jump button for
  `tool === 'advanced' || seeTool`); the phase's tools reordered
  `tlh, montecarlo, equitycomp` to match milestone order (3 = TLH, 4 = MC).
- **RMD spelled out at remaining first mentions** - the milestone-report
  "Plan flags" print line and the 1040 QCD insight (the P07 tool titles
  already spelled it).
- **Em-dash sweep** (foundational rule): TODO / ROADMAP / sprint-log /
  ARCHITECTURE cleaned (306 instances), titles now use "|".

**Roadmapped, not built** (ROADMAP "Industry-advisor feedback", TODO round-22
blocks): smart Retirement goal (auto-link retirement balances), Asset Location
interactivity, documentation gates on milestones, advisor CX playbook, SSN
capture (default-no, safe path specced), Quik!/custodian paperwork automation
(partner-gated), training & onboarding content system. Two decisions sit in
your queue: SSN go/no-go and the Quik!/custodian business contact.
