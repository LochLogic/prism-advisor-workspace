...
Sprint **DocuSign + CSP fix** (see [sprint-log](sprint-log.md)). Build + lint green; app boots clean in preview (the new ack paths need a live DB + DocuSign, so aren't exercisable in the local demo).
- **DocuSign real e-sign** — acknowledgements gain a second provider (`docusign`): advisor escalates a pending ack to a legally-binding envelope (**DocuSign** button in the client modal), client signs via DocuSign email, `docusign-connect` webhook marks it Signed. New: migration **027**, `_shared/docusign.ts` (JWT-grant), `docusign-envelope` + `docusign-connect` functions, `db.sendDocusignEnvelope`, advisor + portal UI, config.toml + deploy.yml wiring. **Inert until activated — see ⚙️ below.**
- **Cloudflare Web Analytics CSP** — `static.cloudflareinsights.com` added to `script-src`, `cloudflareinsights.com` to `connect-src` (`build.mjs`). Ships live with the static site on merge; verify the beacon loads (no CSP console error) on the live site.
- **⚙️ Human activation — DocuSign** (tracked in **H5** below): run migration **027**, set the `DOCUSIGN_*` secrets, grant JWT consent once, run the manual `deploy.yml`, configure DocuSign Connect. Full steps in [`docusign-setup.md`](docusign-setup.md).
- **⚙️ Human deploy — migrations** — ✅ **025 + 026 run 2026-06-07**. **027 pending** (DocuSign).
...
- **DocuSign — ✅ built 2026-06-08, needs activation.** Credentials received; the integration is built (migration 027, `docusign-envelope` + `docusign-connect` functions, advisor + portal UI). **To turn it on, do the steps in [`docusign-setup.md`](docusign-setup.md):**
  1. Run migration **027** in the Supabase SQL editor.
  2. `supabase secrets set` the `DOCUSIGN_*` values (integration key, user id, account id, OAuth base, RSA private key) — from the git-ignored `docs/DocuSign.txt`. Set `DOCUSIGN_CONNECT_HMAC_KEY` for production.
  3. Grant **JWT-grant consent** once (one browser approval — URL in the setup doc), else token requests fail `consent_required`.
  4. Run the manual **Deploy (manual)** workflow (deploys the two new functions).
  5. DocuSign Admin → **Connect** → add the `docusign-connect` URL (JSON Aggregate, Completed event, HMAC).
  *Demo account today (`account-d.docusign.com` / `demo.docusign.net`). For live signatures, promote the account to production + go-live the integration key, and swap `DOCUSIGN_OAUTH_BASE`/`DOCUSIGN_REST_BASE` — sequence with the live-keys decision (H2.3).*
